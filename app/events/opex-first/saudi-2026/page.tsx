                          ...
                          return (
                            <button
                              ...
                              style={{
                                ...
                                padding: "10px 12px",
                                borderRadius: 9,
                                border: active ? `1px solid ${V}66` : "1px solid rgba(255,255,255,0.10)",
                                background: active ? `linear-gradient(135deg, ${V}33, ${V}0f)` : "rgba(255,255,255,0.03)",
                                color: active ? V_BRIGHT : "rgba(255,255,255,0.55)",
                                fontFamily: "var(--font-outfit)",
                                fontSize: 12.5,
                                fontWeight: 600,
                                letterSpacing: "0.2px",
...
// ───────────────────────────────────────────────────────────────────────────
function OpexSaudiPostReportFloat() {
  const [mounted, setMounted] = useState(false);
  ...
  const DISMISS_KEY = "opex-saudi-report-dismissed";
  const NUDGE_KEY = "opex-saudi-report-nudged";
  ...
  const handleOpen = () => {
    window.dispatchEvent(new CustomEvent("opex-saudi:open-request", { detail: { type: "Past Event Report" } }));
    setShowNudge(false);
  };
  ...
  return (
    <>
      <AnimatePresence>
        {showStickyNote && (
          <motion.div
            ...
            className="opex-per-report-note"
            role="button"
            tabIndex={0}
            onClick={handleOpen}
          ...
          >
            <button
              type="button"
              className="opex-per-report-note-close"
            ...
            <span aria-hidden className="opex-per-report-note-hairline" />

            <div className="opex-per-report-note-icon">
            ...
            <div className="opex-per-report-note-body">
              <span className="opex-per-report-note-eyebrow">
                <span className="opex-per-report-note-pulse" aria-hidden />
                Free Download
              </span>
              <span className="opex-per-report-note-title">
                Download our Post Event Reports
              </span>
              <span className="opex-per-report-note-cta">
                View past editions
      ...
      <AnimatePresence>
        {showIcon && (
          <motion.div
            ...
            className="opex-per-report-fab-wrap"
          ...
          >
            <AnimatePresence>
              {showNudge && (
                <motion.div
                  key="nudge"
                  className="opex-per-report-fab-nudge"
                  ...
                  onClick={(e) => e.stopPropagation()}
                >
                  <span className="opex-per-report-fab-nudge-eyebrow">
                    <span className="opex-per-report-note-pulse" aria-hidden /> Free Download
                  </span>
                  <span className="opex-per-report-fab-nudge-text">
                    Download our Post Event Reports
                  </span>
                  <button
                    type="button"
                    className="opex-per-report-fab-nudge-close"
                  ...
                  <span aria-hidden className="opex-per-report-fab-nudge-tail" />
                </motion.div>
              )}
            </AnimatePresence>

            <div className="opex-per-report-fab-btnwrap">
              <button
                type="button"
                className="opex-per-report-fab"
                onClick={handleOpen}
                aria-label="Download Post Event Reports"
                title="Download Post Event Reports"
              >
                <span aria-hidden className="opex-per-report-fab-pulse" />
              ...
              <button
                type="button"
                className="opex-per-report-fab-close"
                onClick={handleDismiss}
                aria-label="Dismiss"
              >
      ...
      <style jsx global>{`
        ...
        .opex-per-report-note-close:hover {
          color: white;
          background: ${V};
          border-color: ${V_BRIGHT};
          transform: rotate(90deg) scale(1.08);
        }
        .opex-per-report-note-icon {
          flex-shrink: 0;
        ...
        .opex-per-report-note-pulse {
          width: 6px; height: 6px; border-radius: 50%;
          background: ${V_BRIGHT};
          box-shadow: 0 0 8px ${V};
          animation: opexPerReportPulse 1.6s ease-in-out infinite;
        }
        .opex-per-report-note-title {
          font-family: var(--font-display);
          font-size: 13.5px;
          font-weight: 700;
          color: white;
        ...
        .opex-per-report-note:hover .opex-per-report-note-cta { color: ${V_BRIGHT}; }
        @keyframes opexPerReportPulse {
          0%, 100% { opacity: 1; transform: scale(1); }
          50%      { opacity: 0.45; transform: scale(1.5); }
        }

        /* ── FAB icon (State B) ────────────────────────────────────────── */
        .opex-per-report-fab-wrap {
          position: fixed;
          bottom: 96px;
          right: 24px;
          z-index: 50;
        ...
        .opex-per-report-fab-pulse {
          position: absolute;
          inset: 0;
          border-radius: 50%;
          background: ${V};
          opacity: 0.4;
          animation: opexPerFabRing 2.4s ease-out infinite;
          z-index: -1;
        }
        @keyframes opexPerFabRing {
          0%   { transform: scale(1);   opacity: 0.55; }
          80%  { transform: scale(1.7); opacity: 0;    }
          100% { transform: scale(1.7); opacity: 0;    }
        }
        .opex-per-report-fab-close {
          position: absolute;
          top: -6px; right: -6px;
        ...
        .opex-per-report-fab-close:hover {
          color: white;
          background: ${V};
          border-color: ${V_BRIGHT};
          transform: rotate(90deg) scale(1.08);
        ...
        .opex-per-report-fab-nudge-eyebrow {
          font-family: var(--font-outfit);
          font-size: 8.5px;
          font-weight: 700;
        ...
        .opex-per-report-fab-nudge-text {
          font-family: var(--font-display);
          font-size: 12.5px;
          font-weight: 700;
          color: white;
        ...
        .opex-per-report-fab-nudge-close {
          position: absolute;
          top: 6px; right: 6px;
          width: 20px; height: 20px;
          display: flex; align-items: center; justify-content: center;
        ...
        .opex-per-report-fab-nudge-close:hover {
          color: white;
          background: ${V}33;
          border-color: ${V}88;
        }
        .opex-per-report-fab-nudge-tail {
          position: absolute;
          right: -6px; top: 50%;
        ...
        @media (max-width: 700px) {
          .opex-per-report-fab-wrap { bottom: 88px; right: 16px; }
          .opex-per-report-fab { width: 50px; height: 50px; }
          .opex-per-report-fab-nudge { max-width: 200px; padding: 10px 28px 10px 12px; }
        }
      `}</style>
    </>
  );
